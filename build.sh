#!/usr/bin/env bash
#
# build.sh - Assemble the US Census Java Calc add-in into build/Census.oxt
#
# Pipeline:
...
#     the Oracle JRE 8 that LibreOffice accepts; see docs/INSTALL.md.
#
# Usage:
#   ./build.sh
...
echo '[1/5] unoidl-write : idl -> build/types/XCensus.rdb'
rdb="$type_dir/XCensus.rdb"
"$uw" "$types" "$root/idl" "$rdb"

# --- 2. Generate Java bindings from the type library ------------------------
echo '[2/5] javamaker    : build/types/XCensus.rdb -> build/gen'
...
echo '[4/5] jar          : classes + bindings -> build/oxt/census.jar'
census_jar="$stage/census.jar"
mf="$root/registration/MANIFEST.MF"
# Merge classes + bindings into one tree first: this JDK's `jar` tool rejects
# duplicate directory entries (e.g. com/example/census/) when given as two
...
"$jar" cfm "$census_jar" "$mf" -C "$jar_dir" .

# --- 5. Stage remaining package files and zip the .oxt ----------------------
echo '[5/5] zip          : staging -> build/Census.oxt'
cp "$rdb" "$stage/types/XCensus.rdb"
...
oxt="$build/Census.oxt"
rm -f "$oxt"
( cd "$stage" && zip -q -r "$oxt" . )

echo
echo "Built $oxt"
echo 'Install with:  unopkg add --force build/Census.oxt   (see docs/INSTALL.md)'
